...
def create_confusion_matrix():
    # real_tags = [41.2, 61.9, 87.6, 97.3, 121.5, 124, 128.3, 138.5, 140.2, 148.2, 175.7, 177.1, 186.5, 207.1, 220.5, 239.5, 280.6, 283.1]
    real_tags = [8.1, 28.4, 41.2, 61.9, 87.6, 97.3, 104.5, 121.5, 124, 128.3, 133.7, 138.5, 140.2, 148.2, 175.7, 177.1, 186.5, 200.2,
                 207.1, 220.5, 228.6, 239.5, 255.2, 280.6, 283.1, 288, 291.4, 292.5, 293.8, 295.5, 299.3]  # bi
    spikes_df = pd.read_csv('../epileptic_activity/402_RAH1-RAH2_bi_spikes.csv')
    raw = mne.io.read_raw_edf('/Users/rotemfalach/Documents/University/lab/EDFs_forRotem/402_for_tag.edf')
    raw.crop(tmin=0, tmax=300)
    # make it 1Hz
    y_actual = np.zeros(int(raw.n_times / 2000), dtype=int)
    y_pred = np.zeros(int(raw.n_times / 2000), dtype=int)
    # floor to 1 sec resolution
    for x in real_tags:
        y_actual[math.floor(x)] = 1
        # y_actual[int(x * 4) - 1: int(x * 4) + 2] = 1
    for i, x in spikes_df.iterrows():
        y_pred[math.floor(x['max_index'] / 2000)] = 1

    df = pd.DataFrame({'pred': y_pred, 'y_actual': y_actual})
    fig, axes = plt.subplots(nrows=1, ncols=2)
    skplt.metrics.plot_confusion_matrix(y_actual, y_pred, ax=axes[1])
    skplt.metrics.plot_confusion_matrix(y_actual, y_pred, ax=axes[0], normalize=True)
    axes[1].images[0].colorbar.ax = axes[0]
...
# save all spikes that cross each other index
...
# save all spikes that the distance from one peak to another spike's peak is less than 50ms
